...
func fetchState(serverInfor ServerInfor) *resty.Response {
	resq, err := resty.R().
		SetQueryParams(map[string]string{
			"metricKeys":   "bugs, duplicated_lines_density, code_smells, alert_status, sqale_index, coverage, tests, test_errors, test_success_density",
...
func convertToNotif(status SonarStatus) (content NotifContent) {
	...
	for _, val := range status.Component.Measures {
		...
		case "tests":
			notifContent.Tests = val.Value
		case "test_errors":
			notifContent.TestsFailed = val.Value
		case "test_success_density":
			notifContent.TestSuccessRatio = val.Value
...
func (notifContent NotifContent) text() string {
	return notifContent.Status + " \n *" +
		...
		"\n *" + notifContent.Tests + "* Tests executed" +
		"\n *" + notifContent.TestsFailed + "* Tests has failed" +
		"\n *" + notifContent.TestSuccessRatio +"%"+ "* Tests were success "
